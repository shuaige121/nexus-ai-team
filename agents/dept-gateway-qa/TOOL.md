...
- `check_inbox.sh dept-gateway-qa` — 检查收件箱，返回未读邮件列表

- `read_mail.sh dept-gateway-qa <mail_id>` — 读取指定邮件的完整内容

## 记忆工具
- `write_memory.sh dept-gateway-qa <key> <value>` — 写入记忆文件，用于持久化重要信息

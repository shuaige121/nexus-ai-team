...
- `check_inbox.sh dept-gateway-manager` — 检查收件箱，返回未读邮件列表

- `read_mail.sh dept-gateway-manager <mail_id>` — 读取指定邮件的完整内容

## 记忆工具
- `write_memory.sh dept-gateway-manager <key> <value>` — 写入记忆文件，用于持久化重要信息
  - 用途: 记录 contract 详情、子任务分配、上下文预算估算、进度跟踪

## 文件工具（只读）
- `ls /home/user/nexus-ai-team/agents/dept-gateway-manager/WORKSPACE` — 查看本部门 Worker 的 WORKSPACE 目录结构
